...
async function getYouTubeAudioUrl(youtubeUrl, userAgent = 'Mozilla/5.0') {
    ...
    try {
        console.log(`[YouTubeStream] Attempting DIRECT yt-dlp: ${youtubeUrl}`);
        ...
        if (audioUrl) {
            console.log(`[YouTubeStream] Got audio URL via direct yt-dlp`);
            return audioUrl;
        }
    } catch (error) {
        console.warn('[YouTubeStream] Direct yt-dlp failed, trying with proxy:', error.message);
    }

    // 2. Fallback to Proxy extraction
    if (cookiePath) {
        try {
...
async function streamYouTubeAudio(youtubeUrl, req, res) {
    ...
    try {
        ...
        // Get direct audio URL with User Agent
        const audioUrl = await getYouTubeAudioUrl(youtubeUrl, userAgent);
